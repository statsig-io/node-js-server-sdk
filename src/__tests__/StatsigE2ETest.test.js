...
describe('Verify e2e behavior of the SDK with mocked network', () => {
    ...
    test('Verify checkGate and exposure logs', async () => {
        ...
        const passingEmail = await statsig.checkGate(statsigUser, 'on_for_statsig_email');
        expect(passingEmail).toEqual(true);
        const failingEmail = await statsig.checkGate(randomUser, 'on_for_statsig_email');
        ...
        expect(postedLogs.events[0].metadata["ruleID"]).toEqual("6N6Z8ODekNYZ7F8gFdoLP5")

        expect(postedLogs.events[1].eventName).toEqual("statsig::gate_exposure")
        expect(postedLogs.events[1].metadata["gate"]).toEqual("on_for_statsig_email")
        expect(postedLogs.events[1].metadata["gateValue"]).toEqual("true")
        expect(postedLogs.events[1].metadata["ruleID"]).toEqual("7w9rbTSffLT89pxqpyhuqK")

        expect(postedLogs.events[2].eventName).toEqual("statsig::gate_exposure")
        expect(postedLogs.events[2].metadata["gate"]).toEqual("on_for_statsig_email")
    ...
    test('Verify getConfig and exposure logs', async () => {
        ...
        expect(postedLogs.events[0].metadata["ruleID"]).toEqual("1kNmlB23wylPFZi1M0Divl")
    ...
    test('Verify getExperiment and exposure logs', async () => {
        const statsig = require('../index');
        await statsig.initialize('secret-123');
        let experiment = await statsig.getExperiment(statsigUser, 'sample_experiment');
        ...
        expect(postedLogs.events.length).toEqual(2);
        expect(postedLogs.events[0].eventName).toEqual("statsig::config_exposure")
        expect(postedLogs.events[0].metadata["config"]).toEqual("sample_experiment")
        expect(postedLogs.events[0].metadata["ruleID"]).toEqual("2RamGujUou6h2bVNQWhtNZ")

        expect(postedLogs.events[1].eventName).toEqual("statsig::config_exposure")
        expect(postedLogs.events[1].metadata["config"]).toEqual("sample_experiment")
        expect(postedLogs.events[1].metadata["ruleID"]).toEqual("2RamGsERWbWMIMnSfOlQuX")
    });

    test('Verify logEvent', async () => {
        const statsig = require('../index');
        await statsig.initialize('secret-123');
        statsig.logEvent(statsigUser, 'add_to_cart', 'SKU_12345', {'price': '9.99', 'item_name': 'diet_coke_48_pack'});
        statsig.shutdown();
        ...
        expect(postedLogs.events[0].metadata['item_name']).toEqual('diet_coke_48_pack')
